...
module.exports = (function (){
  ...
  const o = function ( graph ){
    ...
    this.removePin = function (){
      ...
      const pap = graph.options().pickAndPinModule();
      if ( pap && pap.enabled() ) {
    ...
    this.drawCollapsingButton = function (){
      const collapsingMod = graph.options().collapsingModule();
      const isCollapsed   = collapsingMod && collapsingMod.isCollapsed(that.id());
      ...
      collapsingGroupElement.on("click", ( event ) => {
    ...
    this.drawPinToggleButton = function (){
      const isPinned = that.pinned();
      const dx = (-3.5 / 5) * that.actualRadius();
      const dy = (-7 / 10)  * that.actualRadius();
      ...
      pinGroupElement.on("click", ( event ) => {
        event.stopPropagation();
        if ( isPinned ) {
          that.removePin();
        } else {
          ...
          const papMod = graph.options().pickAndPinModule();
